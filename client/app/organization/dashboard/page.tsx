    ...
    <ul className="space-y-3">
      {sorted.map((t) => (
        <li
          ...
          className="flex items-center justify-between gap-3 text-sm p-2 -mx-2 rounded-md hover:bg-muted/50 transition-colors"
        >
...
function StatusRow({
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-border px-3 py-2.5 transition-colors hover:bg-muted/30">
      ...
      <span className={`inline-flex items-center justify-center rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${bgColors[variant]}`}>
        {formatNumber(value)}
      </span>
    </div>
  );
