...
| `category` | string | `@MaxLength(120)` | no — defaults to `organizations.defaultAppointmentTypeCategory` when set |
...
| `bufferMinutes` | integer | `@Min(0)` | no — default `0` |
| `entityIds` | string[] | `@ArrayUnique()`, UUID ids of bookable persons/resources | no |
...
| `visibility` | `DRAFT` \| `PUBLISHED` \| `ARCHIVED` | optional explicit filter (takes precedence over `published`) |
...
Sets `isPublished = true` and `visibility = PUBLISHED` (share token is generated at create time, and regenerated on demand).
...
Sets `isPublished = false` and `visibility = DRAFT`. Configuration (entities, rules, questions,
shareToken) is preserved — re-publish is cheap.

### POST `/appointment-types/:id/archive` (200)

Sets `isPublished = false` and `visibility = ARCHIVED`. Archived types are
excluded from public discovery and booking flows.

### POST `/appointment-types/:id/unarchive` (200)

Restores an archived type as draft (`visibility = DRAFT`, `isPublished = false`).
...
POST /appointment-types/:id/archive   → visibility=ARCHIVED, not publicly bookable
POST /appointment-types/:id/unarchive → restore as DRAFT
